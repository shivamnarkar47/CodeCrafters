...
class MarketIndex(models.Model):
    ...
    percent_change = models.DecimalField(max_digits=10, decimal_places=4, null=True, blank=True)
    
    def save(self, *args, **kwargs):
        if not self.percent_change:
            try:
                previous = MarketIndex.objects.filter(date__lt=self.date).order_by('-date').first()
                if previous:
                    self.percent_change = ((self.value - previous.value) / previous.value) * 100
            except Exception:
                pass
    ...
    def save(self, *args, **kwargs):
        if not self.percent_change:
            try:
                ...
                if previous:
                    self.percent_change = ((self.closing_price - previous.closing_price) / previous.closing_price) * 100
            except Exception:
                pass
        super().save(*args, **kwargs)
...
class StockPosition(models.Model):
    ...
    user = models.ForeignKey('User', on_delete=models.CASCADE, related_name='stock_positions')
    stock_symbol = models.CharField(max_length=20)
    ...
    def calculate_beta(self, lookback_days=252): 
        stock_changes = list(StockPrice.objects.filter(
            stock_symbol=self.stock_symbol
        ).order_by('-date')[:lookback_days].values_list('percent_change', flat=True))
        
        market_changes = list(MarketIndex.objects.all()
                             .order_by('-date')[:lookback_days].values_list('percent_change', flat=True))
...
class StockTransaction(models.Model):
    ...
    def update_portfolio(self):
        ...
        if self.transaction_type == 'BUY':
            total_cost = (position.quantity * position.average_price) + self.total_amount
